...
import { QuickConfigTabs,PUB_TOPIC } from "../../../../nomenclature/nomenclature"
...
export function validateConfigData(configType, configData) {
    ...
    return new ConfigValidationResult(false, "configType matches none");
}

export function getLambdaPayload(configType, configData, userDetails) {
    
    if (configType == QuickConfigTabs.TAB_USAGE_CHARGE_CONFIG) {
        return getLambdaPayloadForUsageChargeConfig(configData[configType], userDetails);
    }
}
...
export function getPublishTopicName(configType, configData) {
    ...
    var topics = {};
    topics[PUB_TOPIC.CLIENT_TOPIC_GENERIC] = topicPrefixClient + "TOPIC_SSF_READ_CLIENT_TOPIC";
    topics[PUB_TOPIC.CMS_CONFIG_GENERIC] = topicPrefixClient + "TOPIC_SSF_READ_CMS_CONFIG";
    topics[PUB_TOPIC.UCEMS_CONFIG_GENERIC] = topicPrefixClient + "TOPIC_SSF_READ_UCEMS_CONFIG";
    topics[PUB_TOPIC.ODS_CONFIG_GENERIC] = topicPrefixClient + "TOPIC_SSF_READ_ODS_CONFIG";
    topics[PUB_TOPIC.BWT_CONFIG_GENERIC] = topicPrefixClient + "TOPIC_SSF_READ_BWT_CONFIG";
...
function validateUsageChargeConfig(configData) {
    ...
    if (configData === undefined)
        return new ConfigValidationResult(false, "No config parametrs set");
    if (configData.configClient === undefined)
        return new ConfigValidationResult(false, "Config parametr ::Client not selected");
    if (configData.configScope === undefined)
        return new ConfigValidationResult(false, "Config parametr ::Config Scope not selected");

    var isAnyConfigScopeSelected = false;
    Object.keys(configData.configScope).forEach(function (key, index) {
        isAnyConfigScopeSelected = isAnyConfigScopeSelected || configData.configScope[key]
    });
    if (!isAnyConfigScopeSelected)
        return new ConfigValidationResult(false, "Config parametr ::Config Scope not selected, none checked");

    if (configData["id_usageCharge"] === undefined)
        return new ConfigValidationResult(false, "Config parametr ::Usage-Charge not selected");

    if (configData["id_paymentMode"] === undefined)
        return new ConfigValidationResult(false, "Config parametr ::Payment-Mode not selected");
...
function getLambdaPayloadForUsageChargeConfig(configData,userDetails) {
    ...
    selectedCabinTypes.forEach((cabinType, index) => {
        ...
        lambdaPayloadInfo.push(getGenericConfigInfo(
            "UCEMS/USAGE-CHARGE",
            cabinType,
...
function getGenericConfigInfo(configType, cabinType, targetClient, userDetails) {
    var payload = {};
